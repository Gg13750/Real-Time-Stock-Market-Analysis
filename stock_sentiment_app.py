...
st.set_page_config(
    page_title="Real-Time Stock Sentiment Analysis",
    page_icon="📈",
    layout="wide"
)
...
analysis_period = st.sidebar.selectbox("Analysis Period", ["1d", "5d", "1mo", "3mo"])
...
def get_news_headlines(symbol):
    """Fetch news headlines for the stock"""
    try:
        stock = yf.Ticker(symbol)
        news = stock.news
        headlines = []
        for article in news[:10]:  # Get latest 10 news
            headlines.append({
...
if stock_symbol:
    ...
    if hist_data is not None and not hist_data.empty:
        ...
        with col1:
            st.metric(
                label=f"{stock_symbol} Stock Price",
        ...
        if news_data:
            ...
            st.warning("No news data available for this stock symbol.")
            
    else:
        st.error(f"Could not fetch data for {stock_symbol}. Please check the stock symbol.")
